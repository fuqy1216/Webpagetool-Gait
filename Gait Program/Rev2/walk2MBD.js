...
// Not currently used but established variables for arms
var shoulderHeight;
var leftShoulderToElbow;
var rightShoulderToElbow;
var leftElbowToHand;
var rightElbowToHand;
var strideLength = 0;
  ...
  for (var i = 0; i < allTextLines.length; i++) {
    ...
    for (var j = 0; j < data.length; j++) {
      ...
      if (i == 1) {
        ...
        if (j == 1 && data[j] > 0) {
          leftShoulderToElbow = data[j]/2;
        }
        if (j == 2 && data[j] > 0) {
          leftElbowToHand = data[j]/2;
        }
        if (j == 3 && data[j] > 0) {
          leftUpperLegLength = data[j]/2;
          document.getElementById("L1").value = data[j];
        }
        if (j == 4 && data[j] > 0) {
          leftLowerLegLength = data[j]/2;
          document.getElementById("L2").value = data[j];
        }
        if (j == 5 && data[j] > 0) {
          leftAnkleLength = data[j]/2;
          document.getElementById("L3").value = data[j];
        }
        if (j == 6 && data[j] > 0) {
          leftHeelLength = Math.round(data[j]/2*0.16);
          document.getElementById("LHeel").value = leftHeelLength*2;
          leftFootLength = Math.round(data[j]/2*0.66);
          document.getElementById("LFoot").value = leftFootLength*2;
          leftToeLength = Math.round(data[j]/2*0.18);
          document.getElementById("LToe").value = leftToeLength*2;
        }
      }
      if (i == 2) {
        if (j == 0 && data[j] > 0) {
          shoulderHeight = data[j]/2;
        }
        if (j == 1 && data[j] > 0) {
          rightShoulderToElbow = data[j]/2;
        }
        if (j == 2 && data[j] > 0) {
          rightElbowToHand = data[j]/2;
        }
        if (j == 3 && data[j] > 0) {
          rightUpperLegLength = data[j]/2;
          document.getElementById("R1").value = data[j];
        }
        if (j == 4 && data[j] > 0) {
          rightLowerLegLength = data[j]/2;
          document.getElementById("R2").value = data[j];
        }
        if (j == 5 && data[j] > 0) {
          rightAnkleLength = data[j]/2;
          document.getElementById("R3").value = data[j];
        }
        if (j == 6 && data[j] > 0) {
          rightHeelLength = Math.round(data[j]/2*0.16);
          document.getElementById("RHeel").value = leftHeelLength*2;
          rightFootLength = Math.round(data[j]/2*0.66);
          document.getElementById("RFoot").value = leftFootLength*2;
          rightToeLength = Math.round(data[j]/2*0.18);
          document.getElementById("RToe").value = leftToeLength*2;
        }
      }
      if (i > 3) {
...
function highlight(r) {
  ...
  var myRow = myTable.rows[r + 4];
  myRow.style.backgroundColor = '#FFFF00';
}
